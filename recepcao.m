function sinal_recebido = recepcao(sinal_transmissao, param)
    ...
    sinal_recebido.sinal = (exp(1j*4*pi*param.posicao/lambda) .* exp(1j*2*pi*f_doppler*t_recepcao) .* sinal_recepcao + ruido);    
    %sinal_recebido.sinal = (exp(1j*4*pi*param.posicao/lambda) .* sinal_recepcao + ruido);
